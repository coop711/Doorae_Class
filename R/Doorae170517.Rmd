...
box.1 <- ggplot(doorae.ys, aes(x = status, y = years)) + geom_boxplot() + theme_bw() + theme.kr.Malgun
box.2 <- box.1 + ggtitle("공동체상태와 업력") + xlab("공동체상태") + ylab("업력(년)") 
box.3 <- box.2 + scale_x_discrete(labels = c("창업준비", "경영개선")) 
box.4 <- box.3 + annotate("text", x = c(1, 2), y = c(12.2, 13), label = c("예그리나", "아름다운인제관광"), size = 3.5, family ="Malgun Gothic")
box.4
```
...
ecdf.3 <- ecdf.2 + theme_bw() + theme.kr.Malgun
# ecdf.4 <- ecdf.3 + theme(panel.background=element_rect("white"), panel.border=element_rect(colour="black", fill=NA))
ecdf.5 <- ecdf.3 + ggtitle("공동체상태와 업력") 
ecdf.6 <- ecdf.5 + xlab("업력(년)") 
ecdf.7 <- ecdf.6 + ylab("분위") 
ecdf.8 <- ecdf.7 + labs(colour="공동체상태") 
ecdf.9 <- ecdf.8 + scale_colour_discrete(labels=c("창업준비", "경영개선")) 
ecdf.10 <- ecdf.9 + theme(legend.position = c(0.8, 0.5))
ecdf.11 <- ecdf.10 + annotate("segment", x = 3, xend = 6.1, y = 0.5, yend = 0.5, colour = "blue", arrow = arrow(ends = "both", length = unit(0.2, "cm")))
ecdf.12 <- ecdf.11 + scale_x_continuous(breaks = c(0, 1.3, 4.39, 12.3), labels = c(0, 1.3, 4.39,  12.3))
ecdf.13 <- ecdf.12 + annotate("text", x = 4.5, y = 0.54, label = "중위수의 차이", size = 3, family ="Malgun Gothic")
ecdf.14 <- ecdf.13 + annotate("text", x = 4.5, y = 0.46, label = "약 3년", size = 3, family ="Malgun Gothic")
ecdf.14
```
...
poly.2 <- poly.1 + geom_polygon(alpha = 0.3, fill = "red") 
poly.3 <- poly.2 + theme_bw() + theme.kr.Malgun
poly.4 <- poly.3 + theme(panel.background=element_rect("white"), panel.border=element_rect(colour="black", fill=NA))
poly.5 <- poly.4 + ggtitle("공동체상태와 업력")
poly.6 <- poly.5 + xlab("업력(년)")
poly.7 <- poly.6 + ylab("분위") 
poly.8 <- poly.7 + labs(colour="공동체상태") 
poly.9 <- poly.8 + scale_colour_discrete(labels=c("창업준비", "경영개선"))
poly.10 <- poly.9 + theme(legend.position=c(0.8, 0.5))
poly.11 <- poly.10 + scale_x_continuous(breaks=c(0, 1.3, 4.39, 12.3), labels=c(0, 1.3, 4.39, 12.3))
poly.12 <- poly.11 + annotate("text", x=6, y=0.75, label="두 도표 간 면적은\n평균의 차이,\n약 3년", size=4, angle=15, family="Malgun Gothic")
poly.13 <- poly.12 + annotate("text", x=9, y=0.6, label="경영개선", size=4, family="Malgun Gothic")
poly.14 <- poly.13 + annotate("text", x=2, y=0.9, label="창업준비", size=4, family="Malgun Gothic")
poly.14
```
...
d.3 <- d.2 + scale_colour_brewer(palette="Set1", limits=c("경영개선", "창업준비"))
d.4 <- d.3 + theme_bw()
d.5 <- d.4 + theme.kr.Malgun
d.6 <- d.5 + ylab("조직명")  
d.7 <- d.6 + theme(axis.text.y=element_text(size=6)) 
d.8 <- d.7 + theme(panel.grid.major.y=element_blank(), legend.position = c(0.9, 0.55), 
legend.justification=c(1, 0.5))
d.9 <- d.8 + ggtitle("공동체상태별 업력") 
d.9
```
